...
datumGen::datumGen(): Node("DatumGen")
{
  ...
  this->declare_parameter("odom_topic","/hunter/odom");
  this->declare_parameter("gps_topic","/hunter/fix");
  this->declare_parameter("cmd_vel_topic","/cmd_vel");
  this->declare_parameter("global_imu_topic","/odometry_imu");
  this->declare_parameter("madgwick_imu_topic","/imu_complementary");
  this->declare_parameter("distance_to_move",2);
  this->declare_parameter("speed",0.8);
  this->declare_parameter("publish_global_imu",true);
  ...
  if(debug){
    pub_debug_or = this->create_publisher<geometry_msgs::msg::Point>("/debug_rpy_imu",10);
    pub_debug_hunter_or = this->create_publisher<geometry_msgs::msg::Point>("/debug_rpy_hunter",10);

    sub_debug_odom = this->create_subscription<nav_msgs::msg::Odometry>(
          "/hunter/odom",10, std::bind(&datumGen::odomDebugCallback,this,_1));
  }
...
void datumGen::odomDebugCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  tf2::Quaternion q;
  ...
  if(debug){
      geometry_msgs::msg::Point rpy_point;
      rpy_point.x = roll;
      rpy_point.y = pitch;
      rpy_point.z = yaw;
      pub_debug_hunter_or->publish(rpy_point);
    }
}
...
void datumGen::madgwickImuCallback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
  if(datum_sended)
  {
    ...
    // For now we create the covariance matrix as a constant, a huge value so it defenetly we "use it" in the ekf
    ...
    if(debug){
      ...
      rpy_point.y = pitch;
      rpy_point.z = yaw_msg;
      pub_debug_or->publish(rpy_point);
    }
  }
}
